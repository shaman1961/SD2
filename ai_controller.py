...
class AIController:
    ...
    def __init__(self, country_name: str, country_data: dict, provinces_data: dict,
        ...
        self.provinces_data = provinces_data  # {name: {...}}
        self.player_country_name = player_country_name
        self.all_armies = all_armies  # {prov_name: {"owner": id, "count": N} или просто owner_id}

        self.gold = country_data.get('gold', 100)
        self.provinces = country_data.get('provinces', [])
        self.color = country_data.get('color', [0, 0, 0])

        # Армии бота: имена провинций и количество дивизий
        self.armies = []  # Список имён провинций где есть армия бота
        self.army_counts = {}  # {prov_name: count}

        for name, info in all_armies.items():
            # Поддерживаем и старый формат (просто owner_id), и новый (словарь)
    ...
    def make_move(self, game=None) -> dict:
        ...
        if self.gold >= 50 and self.provinces:
            for prov_name in self.provinces:
                ...
                if owner == self.country_name:
                    # Уже есть армия в провинции — увеличиваем количество
                    if isinstance(info, dict):
                        info["count"] = count + 1
                    else:
        ...
        # === ПРИОРИТЕТ 2: Атаковать соседа ===
        if self.armies:
            # Сортируем по убыванию количества дивизий (атакуем сильнейшими)
            ...
            for army_prov in attack_order:
                ...
                for neighbor_name in neighbors:
                    ...
                    # Атакуем только чужие провинции где есть вражеская армия
                    if neighbor_owner and neighbor_owner != self.country_name:
                        if my_count > neighbor_count:
                            # ПОБЕДА: атакуем!
                            losses = max(0, int(neighbor_count * 0.7))  # Базовые потери 70% от врага
                            survivors = max(1, my_count - losses)

                            # Обновляем целевую провинцию
                            ...
                            # Убираем армию из исходной провинции
                        ...
                        # Если my_count < neighbor_count — не атакуем, ищем другого соседа

        # === ПРИОРИТЕТ 3: Инвестировать в ресурс ===
        ...
        # Обновляем золото в country_data
